module.exports = grammar({
  ...
  rules: {
    ...
    inst: ($) => choice("active", seq("active", "[", $.const_expr, "]"), seq("active", "[", $.uname, "]")),
    ...
    break: (_$) => "break",
    goto: ($) => seq("goto", $.uname),
    rarg: ($) => choice($.varref, seq("eval", "(", $.expr, ")"), $.const, seq("-", $.const)),
    rargs: ($) =>
      prec.left(1, choice($.rarg, seq($.rarg, ",", $.rargs), seq($.rarg, "(", $.rargs, ")"), seq("(", $.rargs, ")"))),
    if: ($) => seq("if", $.options, "fi"),
    do: ($) => seq("do", $.options, "od"),
    for_pre: ($) => seq("for", "(", $.varref),
    for_post: ($) =>
      choice(seq("{", $.sequence, optional($._semi), "}"), seq($._semi, "{", $.sequence, optional($._semi), "}")),
    ...
    type: (_$) => choice("bit", "bool", "byte", "chan", "int", "mtype", "pid", "short"),
    ...
    number: (_$) => /\d+/,
    comment: (_$) => token(choice(seq("//", /.*/), seq("/*", repeat(choice(/[^*]/, /\*[^/]/)), "*/"))),
  },
});
