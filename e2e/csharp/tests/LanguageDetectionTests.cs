// This file is auto-generated by alef — DO NOT EDIT.
// To regenerate: alef generate
// To verify freshness: alef verify

using System;
using System.Collections.Generic;
using System.Linq;
using System.Net.Http;
using System.Text;
using System.Text.Json;
using System.Text.Json.Serialization;
using System.Threading.Tasks;
using Xunit;
using TreeSitterLanguagePack;
using static TreeSitterLanguagePack.TreeSitterLanguagePackConverter;

namespace TreeSitterLanguagePack {
/// <summary>E2e tests for category: language-detection.</summary>
public class LanguageDetectionTests {
  private static readonly JsonSerializerOptions ConfigOptions =
      new() { Converters = { new JsonStringEnumConverter(
                  JsonNamingPolicy.SnakeCaseLower) },
              DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingDefault };

  [Fact]
  public void Test_DetectContentBashShebang() {
    // detect_language_from_content recognizes #!/bin/bash shebang
    var result = TreeSitterLanguagePackConverter.DetectLanguageFromContent(
        "#!/bin/bash\necho hi");
    Assert.Equal("bash", result);
  }

  [Fact]
  public void Test_DetectContentNoShebang() {
    // detect_language_from_content returns None when no shebang present
    var result = TreeSitterLanguagePackConverter.DetectLanguageFromContent(
        "no shebang here");
    Assert.True(string.IsNullOrEmpty(result?.ToString()));
  }

  [Fact]
  public void Test_DetectContentPythonShebang() {
    // detect_language_from_content recognizes #!/usr/bin/env python3 shebang
    var result = TreeSitterLanguagePackConverter.DetectLanguageFromContent(
        "#!/usr/bin/env python3\npass");
    Assert.Equal("python", result);
  }

  [Fact]
  public void Test_DetectExtCpp() {
    // detect_language_from_extension recognizes .cpp as cpp
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromExtension("cpp");
    Assert.Equal("cpp", result);
  }

  [Fact]
  public void Test_DetectExtGherkin() {
    // detect_language_from_extension recognizes .feature as gherkin
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromExtension("feature");
    Assert.Equal("gherkin", result);
  }

  [Fact]
  public void Test_DetectExtGo() {
    // detect_language_from_extension recognizes .go as go
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromExtension("go");
    Assert.Equal("go", result);
  }

  [Fact]
  public void Test_DetectExtJava() {
    // detect_language_from_extension recognizes .java as java
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromExtension("java");
    Assert.Equal("java", result);
  }

  [Fact]
  public void Test_DetectExtJavascript() {
    // detect_language_from_extension recognizes .js as javascript
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromExtension("js");
    Assert.Equal("javascript", result);
  }

  [Fact]
  public void Test_DetectExtPhp() {
    // detect_language_from_extension recognizes .php as php
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromExtension("php");
    Assert.Equal("php", result);
  }

  [Fact]
  public void Test_DetectExtPython() {
    // detect_language_from_extension recognizes .py as python
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromExtension("py");
    Assert.Equal("python", result);
  }

  [Fact]
  public void Test_DetectExtRuby() {
    // detect_language_from_extension recognizes .rb as ruby
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromExtension("rb");
    Assert.Equal("ruby", result);
  }

  [Fact]
  public void Test_DetectExtRust() {
    // detect_language_from_extension recognizes .rs as rust
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromExtension("rs");
    Assert.Equal("rust", result);
  }

  [Fact]
  public void Test_DetectExtTypescript() {
    // detect_language_from_extension recognizes .ts as typescript
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromExtension("ts");
    Assert.Equal("typescript", result);
  }

  [Fact]
  public void Test_DetectExtUnknown() {
    // detect_language_from_extension returns None for unknown extension .xyz
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromExtension("xyz");
    Assert.True(string.IsNullOrEmpty(result?.ToString()));
  }

  [Fact]
  public void Test_DetectPathDotfile() {
    // detect_language_from_path returns None for dotfile .gitignore (no
    // standard extension)
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromPath(".gitignore");
    Assert.True(string.IsNullOrEmpty(result?.ToString()));
  }

  [Fact]
  public void Test_DetectPathGoNested() {
    // detect_language_from_path extracts extension from nested path
    // lib/server.go
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromPath("lib/server.go");
    Assert.Equal("go", result);
  }

  [Fact]
  public void Test_DetectPathJavaRoot() {
    // detect_language_from_path recognizes Main.java in root directory
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromPath("Main.java");
    Assert.Equal("java", result);
  }

  [Fact]
  public void Test_DetectPathJsRoot() {
    // detect_language_from_path recognizes app.js in root directory
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromPath("app.js");
    Assert.Equal("javascript", result);
  }

  [Fact]
  public void Test_DetectPathNested() {
    // detect_language_from_path extracts extension from nested path src/main.py
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromPath("src/main.py");
    Assert.Equal("python", result);
  }

  [Fact]
  public void Test_DetectPathNoExtension() {
    // detect_language_from_path returns None for extensionless file Makefile
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromPath("Makefile");
    Assert.True(string.IsNullOrEmpty(result?.ToString()));
  }

  [Fact]
  public void Test_DetectPathRustSrc() {
    // detect_language_from_path extracts extension from nested path src/main.rs
    var result =
        TreeSitterLanguagePackConverter.DetectLanguageFromPath("src/main.rs");
    Assert.Equal("rust", result);
  }
}
}
