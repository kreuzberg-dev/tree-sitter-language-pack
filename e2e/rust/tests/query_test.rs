...
fn test_run_query_no_matches() {
    ...
    assert!(!tree_sitter_language_pack::run_query(&tree, "python", r#"(class_definition name: (identifier) @name)"#, source.as_bytes()).unwrap().is_empty(), "expected >= 0");
}
...
"#;
    let tree = parse_string(&language, source.as_bytes()).expect("should succeed");
    assert!(tree_sitter_language_pack::run_query(&tree, "python", r#"(function_definition name: (identifier) @name)"#, source.as_bytes()).unwrap().len() >= 2, "expected at least 2 elements, got {}", tree_sitter_language_pack::run_query(&tree, "python", r#"(function_definition name: (identifier) @name)"#, source.as_bytes()).unwrap().len());
}
