...
// alef:hash:5483df51ea5abdb4624924f9d7e960a4128c7f5ccabe25b0fa38d9fab6f74e16
...
"#;
    ...
    assert_eq!(result.language.to_string(), r#"c"#, "equals assertion failed");
    assert!(!result.structure.is_empty(), "expected >= 1");
    assert!(
        result
            .structure
            .iter()
            .any(|e| format!("{:?}", e.structure[0].kind).contains(r#"Function"#)),
        "expected some element of structure[].kind to contain: {}",
        r#"Function"#
    );
...
"#;
    ...
    let config = serde_json::from_value(config_json).unwrap();
    let result = process(source, &config).expect("call failed");
    assert_eq!(result.language.to_string(), r#"python"#, "equals assertion failed");
    assert!(!result.structure.is_empty(), "expected >= 1");
    assert!(
        result
            .structure
...
"#;
    ...
    let config = serde_json::from_value(config_json).unwrap();
    let result = process(source, &config).expect("call failed");
    assert_eq!(result.language.to_string(), r#"go"#, "equals assertion failed");
    assert!(!result.structure.is_empty(), "expected >= 1");
    assert!(
        result
            .structure
...
"#;
    ...
    let config = serde_json::from_value(config_json).unwrap();
    let result = process(source, &config).expect("call failed");
    assert_eq!(result.language.to_string(), r#"java"#, "equals assertion failed");
    assert!(!result.structure.is_empty(), "expected >= 1");
    assert!(
        result
            .structure
            .iter()
            .any(|e| format!("{:?}", e.structure[0].kind).contains(r#"Class"#)),
        "expected some element of structure[].kind to contain: {}",
        r#"Class"#
    );
    assert!(!result.imports.is_empty(), "expected >= 1");
...
"#;
    ...
    assert!(
        result
            .structure
            .iter()
            .any(|e| format!("{:?}", e.structure[0].kind).contains(r#"Module"#)),
        "expected some element of structure[].kind to contain: {}",
        r#"Module"#
    );
    assert!(
...
"#;
    ...
    let config = serde_json::from_value(config_json).unwrap();
    let result = process(source, &config).expect("call failed");
    assert_eq!(result.language.to_string(), r#"javascript"#, "equals assertion failed");
    assert!(!result.structure.is_empty(), "expected >= 1");
    assert!(
        result
            .structure
...
"#;
    ...
    let config = serde_json::from_value(config_json).unwrap();
    let result = process(source, &config).expect("call failed");
    assert_eq!(result.language.to_string(), r#"kotlin"#, "equals assertion failed");
    assert!(
        result.structure.len() >= 2,
        "expected at least 2 elements, got {}",
        result.structure.len()
...
"#;
    ...
    assert!(
        result
            .imports
            .iter()
            .any(|e| e.imports[0].source.to_string().contains(r#"os"#)),
        "expected some element of imports[].source to contain: {}",
        r#"os"#
    );
}
...
"#;
    ...
    let config = serde_json::from_value(config_json).unwrap();
    let result = process(source, &config).expect("call failed");
    assert_eq!(result.language.to_string(), r#"rust"#, "equals assertion failed");
    assert!(!result.structure.is_empty(), "expected >= 1");
    assert!(
        result.structure.iter().any(
            |item| serde_json::to_value(item).ok().is_some_and(|value| match &value {
                serde_json::Value::String(text) => text.contains(r#"MyConfig"#),
                serde_json::Value::Object(fields) =>
                    ["kind", "name", "source", "alias", "text", "signature"]
                        ...
                        .any(|key| fields
                            ...
                            .is_some_and(|text| text.contains(r#"MyConfig"#)))
                        || value.to_string().contains(r#"MyConfig"#),
                _ => false,
            })
        ),
...
"#;
    ...
    let config = serde_json::from_value(config_json).unwrap();
    let result = process(source, &config).expect("call failed");
    assert_eq!(result.language.to_string(), r#"ruby"#, "equals assertion failed");
    assert!(!result.structure.is_empty(), "expected >= 1");
    assert!(
        result
            .structure
...
"#;
    ...
    let config = serde_json::from_value(config_json).unwrap();
    let result = process(source, &config).expect("call failed");
    assert_eq!(result.language.to_string(), r#"typescript"#, "equals assertion failed");
    assert!(!result.structure.is_empty(), "expected >= 1");
    assert!(
        result
            .structure
