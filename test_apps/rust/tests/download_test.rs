...
// alef:hash:53a0431e64410948449fbf1d836d3f51a9b32fdfb5f059dd2c96f7157bea10ec
...
fn test_download_clean_cache() {
    ...
    let _ = clean_cache().expect("call failed");
}
...
fn test_download_configure_custom_dir() {
    ...
    let _ = configure(&config).expect("call failed");
}
...
fn test_download_init_default() {
    ...
    let config = serde_json::from_value(config_json).unwrap();
    let _ = init(&config).expect("call failed");
}
...
fn test_prefetch_empty_list() {
    ...
    let _ = prefetch(&languages_refs).expect("call failed");
}
