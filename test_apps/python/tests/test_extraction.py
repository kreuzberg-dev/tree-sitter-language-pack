...
def test_extract_invalid_byte_range() -> None:
    ...
    config = {
        "language": "python",
        "patterns": {
            "funcs": {"byte_range": [9999, 99999], "capture_output": "Full", "query": "(expression_statement) @e"}
        },
    }
...
def test_extract_patterns_python_functions() -> None:
    ...
    config = {
        "language": "python",
        "patterns": {
            "functions": {"capture_output": "Full", "query": "(function_definition name: (identifier) @name)"}
        },
    }
    result = extract_patterns(source=source, config=config)
