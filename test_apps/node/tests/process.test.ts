...
// alef:hash:25fd1ca1f41320372fb476f8c16316b22c0b195b9a211dc54ca7e56fd6185aff
...
describe("process", () => {
  it("c_function_process: Intel: C function with include", () => {
    ...
    expect((result.structure ?? []).some((e) => String(e.structure[0].kind).includes("Function"))).toBe(true);
  ...
  it("java_class_process: Intel: Java class with methods and imports", () => {
    ...
    expect(result.structure.length).toBeGreaterThanOrEqual(1);
    expect((result.structure ?? []).some((e) => String(e.structure[0].kind).includes("Class"))).toBe(true);
    expect(result.imports.length).toBeGreaterThanOrEqual(1);
    expect(result.metrics.totalLines).toBeGreaterThanOrEqual(7);
    expect(result.metrics.errorCount).toBe(0);
  }, 30000);
  ...
  it("java_package_declaration_process: Intel: Java package declaration emitted as Module so callers can build FQNs (#112)", () => {
    ...
    expect((result.structure ?? []).some((e) => String(e.structure[0].kind).includes("Module"))).toBe(true);
    expect((result.structure ?? []).some((e) => String(e.structure[0].kind).includes("Class"))).toBe(true);
    expect(result.metrics.errorCount).toBe(0);
  }, 30000);
  ...
  it("process_python_imports_detail: Python with multiple imports, verify imports contain specific source", () => {
    ...
    expect(result.language ?? "").toBe("python");
    expect(result.imports.length).toBeGreaterThanOrEqual(2);
    expect((result.imports ?? []).some((e) => String(e.imports[0].source).includes("os"))).toBe(true);
  }, 30000);
