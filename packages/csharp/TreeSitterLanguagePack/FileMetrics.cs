    ...
    try {
      return JsonSerializer.Deserialize<FileMetrics>(json, JsonOptions) ??
             throw new TreeSitterLanguagePackException(
                 $"Failed to parse FileMetrics from JSON: deserializer returned null");
    } catch (TreeSitterLanguagePackException) {
      throw;
    } catch (Exception e) {
      throw new TreeSitterLanguagePackException(
          $"Failed to parse FileMetrics from JSON: {e.Message}", e);
    }
  }
