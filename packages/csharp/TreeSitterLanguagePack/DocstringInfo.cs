...
public sealed record DocstringInfo {
  ...
  public List<DocSection> ParsedSections {
    get; init;
  } = [];
    ...
    try {
      return JsonSerializer.Deserialize<DocstringInfo>(json, JsonOptions) ??
             throw new TreeSitterLanguagePackException(
                 $"Failed to parse DocstringInfo from JSON: deserializer returned null");
    } catch (TreeSitterLanguagePackException) {
      throw;
    } catch (Exception e) {
      throw new TreeSitterLanguagePackException(
          $"Failed to parse DocstringInfo from JSON: {e.Message}", e);
    }
  }
