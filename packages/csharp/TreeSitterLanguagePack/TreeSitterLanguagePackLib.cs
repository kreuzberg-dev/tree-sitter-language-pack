...
// alef:hash:b4d0026bf0a62cf79b723ebfe265853a7c145959357398010ea4c946311989e2
...
public static class TreeSitterLanguagePackLib
{
    ...
    public static NodeInfo RootNodeInfo(Tree tree)
    {
        ...
        var nativeResult = NativeMethods.RootNodeInfo(
            tree.Handle
        );
        if (nativeResult == IntPtr.Zero)
        {
        ...
        var jsonPtr = NativeMethods.NodeInfoToJson(nativeResult);
        var json = Marshal.PtrToStringUTF8(jsonPtr);
        NativeMethods.FreeString(jsonPtr);
        NativeMethods.NodeInfoFree(nativeResult);
    ...
    public static List<NodeInfo> FindNodesByType(Tree tree, string nodeType)
    {
        ...
        var nativeResult = NativeMethods.FindNodesByType(
            tree.Handle,
            nodeType
        );
        if (nativeResult == IntPtr.Zero)
    ...
    {
        ArgumentNullException.ThrowIfNull(tree);
        var nativeResult = NativeMethods.NamedChildrenInfo(
            tree.Handle
        );
        if (nativeResult == IntPtr.Zero)
        {
    ...
    public static bool TreeContainsNodeType(Tree tree, string nodeType)
    {
        ArgumentNullException.ThrowIfNull(tree);
        ArgumentNullException.ThrowIfNull(nodeType);
        var nativeResult = NativeMethods.TreeContainsNodeType(
            tree.Handle,
            nodeType
        );
    ...
    {
        ArgumentNullException.ThrowIfNull(tree);
        var nativeResult = NativeMethods.TreeHasErrorNodes(
            tree.Handle
        );
        var returnValue = nativeResult != 0;
        return returnValue;
    ...
    {
        ArgumentNullException.ThrowIfNull(tree);
        var nativeResult = NativeMethods.TreeToSexp(
            tree.Handle
        );
        if (nativeResult == IntPtr.Zero)
        {
    ...
    {
        ArgumentNullException.ThrowIfNull(tree);
        var nativeResult = NativeMethods.TreeErrorCount(
            tree.Handle
        );
    ...
    public static List<QueryMatch> RunQuery(Tree tree, string language, string querySource, byte[] source)
    {
        ...
        ArgumentNullException.ThrowIfNull(source);
        var sourceHandle = GCHandle.Alloc(source, GCHandleType.Pinned);
        var nativeResult = NativeMethods.RunQuery(
            tree.Handle,
            language,
            querySource,
            sourceHandle.AddrOfPinnedObject()
    ...
    public static ExtractionResult ExtractPatterns(string source, ExtractionConfig config)
    {
        ...
        var nativeResult = NativeMethods.ExtractPatterns(
            source,
            configHandle
        );
        if (nativeResult == IntPtr.Zero)
    ...
    public static ValidationResult ValidateExtraction(ExtractionConfig config)
    {
        ...
        var configJson = JsonSerializer.Serialize(config, JsonOptions);
        var configHandle = NativeMethods.ExtractionConfigFromJson(configJson);
        var nativeResult = NativeMethods.ValidateExtraction(
            configHandle
        );
        if (nativeResult == IntPtr.Zero)
        {
        ...
        var jsonPtr = NativeMethods.ValidationResultToJson(nativeResult);
        var json = Marshal.PtrToStringUTF8(jsonPtr);
        NativeMethods.FreeString(jsonPtr);
        NativeMethods.ValidationResultFree(nativeResult);
