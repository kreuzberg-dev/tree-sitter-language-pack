...
public sealed record PackConfig {
  ...
  /// <summary>
  /// Language groups to pre-download (e.g. `"web"`, `"systems"`,
  /// `"scripting"`).
  /// </summary>
    ...
    try {
      return JsonSerializer.Deserialize<PackConfig>(json, JsonOptions) ??
             throw new TreeSitterLanguagePackException(
                 $"Failed to parse PackConfig from JSON: deserializer returned null");
    } catch (TreeSitterLanguagePackException) {
      throw;
    } catch (Exception e) {
      throw new TreeSitterLanguagePackException(
          $"Failed to parse PackConfig from JSON: {e.Message}", e);
    }
  }
