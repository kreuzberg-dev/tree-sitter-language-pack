...
public sealed record StructureItem {
  ...
  /// <summary>
  /// Full signature text of the item (e.g., function parameters and return
  /// type).
  /// </summary>
  ...
  /// <summary>
  /// Source span covering only the body of the item, if distinct from the
  /// declaration.
  /// </summary>
    ...
    try {
      return JsonSerializer.Deserialize<StructureItem>(json, JsonOptions) ??
             throw new TreeSitterLanguagePackException(
                 $"Failed to parse StructureItem from JSON: deserializer returned null");
    } catch (TreeSitterLanguagePackException) {
      throw;
    } catch (Exception e) {
      throw new TreeSitterLanguagePackException(
          $"Failed to parse StructureItem from JSON: {e.Message}", e);
    }
  }
