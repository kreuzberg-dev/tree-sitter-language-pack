...
public enum DataNodeKind {
  ...
  /// </summary>
  [JsonPropertyName("KeyValue")] KeyValue,
  /// <summary>
...
/// <summary>
/// Custom JSON converter for <see cref="DataNodeKind"/> that respects explicit
/// variant names.
/// </summary>
internal sealed class DataNodeKindJsonConverter : JsonConverter<DataNodeKind> {
  public override DataNodeKind Read(ref Utf8JsonReader reader,
                                    Type typeToConvert,
                                    JsonSerializerOptions options) {
    var value = reader.GetString();
    return value switch {
      "KeyValue" => DataNodeKind.KeyValue, "Element" => DataNodeKind.Element,
      "Sequence" => DataNodeKind.Sequence,
      _ => throw new JsonException($"Unknown DataNodeKind value: {value}")
    };
  }

  public override void Write(Utf8JsonWriter writer, DataNodeKind value,
                             JsonSerializerOptions options) {
    var str = value switch {
      DataNodeKind.KeyValue => "KeyValue", DataNodeKind.Element => "Element",
