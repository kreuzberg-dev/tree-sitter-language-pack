...
public sealed record ProcessConfig {
  ...
  /// <summary>
  /// Extract hierarchical key/value data tree from data-format files. Default:
  /// false.
  ///
  ...
  /// </summary>
  /// <exception cref="TreeSitterLanguagePackException">When the JSON cannot be
  ...
  public static ProcessConfig FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<ProcessConfig>(json, JsonOptions) ??
             throw new TreeSitterLanguagePackException(
                 $"Failed to parse ProcessConfig from JSON: deserializer returned null");
    } catch (TreeSitterLanguagePackException) {
      throw;
    } catch (Exception e) {
      throw new TreeSitterLanguagePackException(
          $"Failed to parse ProcessConfig from JSON: {e.Message}", e);
    }
  }
  ...
  public ProcessConfig WithChunking(ulong maxSize) {
    ...
    try {
      var nativeResult =
          NativeMethods.ProcessConfigWithChunking(selfHandle, maxSize);
      var jsonPtr = NativeMethods.ProcessConfigToJson(nativeResult);
  ...
  public ProcessConfig WithDataExtraction(bool enabled) {
    ...
    try {
      var nativeResult =
          NativeMethods.ProcessConfigWithDataExtraction(selfHandle, enabled);
      var jsonPtr = NativeMethods.ProcessConfigToJson(nativeResult);
      var json = global::System.Runtime.InteropServices.Marshal.PtrToStringUTF8(
          jsonPtr);
      NativeMethods.FreeString(jsonPtr);
