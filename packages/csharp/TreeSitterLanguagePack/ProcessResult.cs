// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:c67517c6eb3fd3461ef481e59b7717df319a492db9aea10017c9e8f71d249c87
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code
#nullable enable

using System;
using System.Collections.Generic;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace TreeSitterLanguagePack;

/// <summary>
/// Complete analysis result from processing a source file.
///
/// Contains metrics, structural analysis, imports/exports, comments,
/// docstrings, symbols, diagnostics, and optionally chunked code segments.
/// Fields are populated based on the `ProcessConfig` flags.
///
/// # Fields
///
/// - `language` - The language used for parsing
/// - `metrics` - Always computed: line counts, byte sizes, error counts
/// - `structure` - Functions, classes, structs (when `config.structure = true`)
/// - `imports` - Import statements (when `config.imports = true`)
/// - `exports` - Export statements (when `config.exports = true`)
/// - `comments` - Comments (when `config.comments = true`)
/// - `docstrings` - Docstrings (when `config.docstrings = true`)
/// - `symbols` - Symbol definitions (when `config.symbols = true`)
/// - `diagnostics` - Parse errors (when `config.diagnostics = true`)
/// - `chunks` - Chunked code segments (when `config.chunk_max_size` is set)
/// </summary>
public sealed record ProcessResult {
  /// <summary>
  /// The language name used to parse the source file.
  /// </summary>
  [JsonPropertyName("language")]
  public string Language { get; init; } = "";

  /// <summary>
  /// File-level metrics (line counts, byte size, error count).
  /// </summary>
  [JsonPropertyName("metrics")]
  public FileMetrics Metrics {
    get; init;
  } = default!;

  /// <summary>
  /// Top-level structural items (functions, classes, etc.).
  /// </summary>
  [JsonPropertyName("structure")]
  public List<StructureItem> Structure {
    get; init;
  } = [];

  /// <summary>
  /// Import statements extracted from the source.
  /// </summary>
  [JsonPropertyName("imports")]
  public List<ImportInfo> Imports {
    get; init;
  } = [];

  /// <summary>
  /// Export statements extracted from the source.
  /// </summary>
  [JsonPropertyName("exports")]
  public List<ExportInfo> Exports {
    get; init;
  } = [];

  /// <summary>
  /// Comments extracted from the source.
  /// </summary>
  [JsonPropertyName("comments")]
  public List<CommentInfo> Comments {
    get; init;
  } = [];

  /// <summary>
  /// Docstrings extracted from the source.
  /// </summary>
  [JsonPropertyName("docstrings")]
  public List<DocstringInfo> Docstrings {
    get; init;
  } = [];

  /// <summary>
  /// Symbol definitions (variables, types, functions) extracted from the
  /// source.
  /// </summary>
  [JsonPropertyName("symbols")]
  public List<SymbolInfo> Symbols {
    get; init;
  } = [];

  /// <summary>
  /// Parse diagnostics (syntax errors, missing nodes) from tree-sitter.
  /// </summary>
  [JsonPropertyName("diagnostics")]
  public List<Diagnostic> Diagnostics {
    get; init;
  } = [];

  /// <summary>
  /// Syntax-aware code chunks produced when chunking is enabled.
  /// </summary>
  [JsonPropertyName("chunks")]
  public List<CodeChunk> Chunks {
    get; init;
  } = [];

  /// <summary>
  /// Hierarchical data tree extracted when `config.data_extraction` is `true`.
  ///
  /// Populated for supported data-format languages (JSON, YAML, TOML,
  /// properties, HCL, INI, XML, CSV, and more). `None` when `data_extraction`
  /// is `false` (the default) or when the language is not a recognised data
  /// format.
  ///
  /// See `DataNode` for the shape of the returned tree.
  /// </summary>
  [JsonPropertyName("data")]
  public DataNode
      ? Data {
          get; init;
        } = null;

  /// <summary>
  /// Parse a <see cref="ProcessResult"/> from a JSON string.
  /// </summary>
  /// <exception cref="TreeSitterLanguagePackException">When the JSON cannot be
  /// deserialised.</exception>
  public static ProcessResult FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<ProcessResult>(json, JsonOptions) ??
             throw new TreeSitterLanguagePackException(
                 $"Failed to parse ProcessResult from JSON: deserializer returned null");
    } catch (TreeSitterLanguagePackException) {
      throw;
    } catch (Exception e) {
      throw new TreeSitterLanguagePackException(
          $"Failed to parse ProcessResult from JSON: {e.Message}", e);
    }
  }

  private static readonly JsonSerializerOptions JsonOptions = new() {
    DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingDefault,
    Converters = { new JsonStringEnumConverter(JsonNamingPolicy.SnakeCaseLower),
                   new ByteArrayJsonConverter() },
  };

  /// <summary>Options for serializing config/input objects to FFI. Strips nulls
  /// (nullable C# fields default to null and would override required Rust
  /// fields with non-deserialisable nulls) but preserves explicit false/0 so
  /// caller intent is kept.</summary>
  private static readonly JsonSerializerOptions JsonSerializationOptions =
      new() {
        DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull,
        Converters = { new JsonStringEnumConverter(
                           JsonNamingPolicy.SnakeCaseLower),
                       new ByteArrayJsonConverter() },
      };
}
