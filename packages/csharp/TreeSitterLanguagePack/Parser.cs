// This file is auto-generated by alef. DO NOT EDIT.
// alef:hash:c67517c6eb3fd3461ef481e59b7717df319a492db9aea10017c9e8f71d249c87
#nullable enable

using System;
using Microsoft.Win32.SafeHandles;
using System.Runtime.InteropServices;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace TreeSitterLanguagePack;

internal sealed class ParserSafeHandle : SafeHandle {
  internal ParserSafeHandle(IntPtr handle) : base(IntPtr.Zero, true) {
    SetHandle(handle);
  }

  public override bool IsInvalid => handle == IntPtr.Zero;

  protected override bool ReleaseHandle() {
    NativeMethods.ParserFree(handle);
    return true;
  }
}
/// <summary>/// A tree-sitter parser configured for one language at a time.///
/// </summary>
public sealed class Parser : IDisposable {
  private static readonly JsonSerializerOptions JsonOptions =
      new() { Converters = { new JsonStringEnumConverter(
                  JsonNamingPolicy.SnakeCaseLower) },
              DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingDefault };

  private static readonly JsonSerializerOptions JsonSerializationOptions =
      new() {
        Converters = { new JsonStringEnumConverter(
            JsonNamingPolicy.SnakeCaseLower) },
      };
  private readonly ParserSafeHandle _safeHandle;

  internal Parser(IntPtr handle) { _safeHandle = new ParserSafeHandle(handle); }

  internal IntPtr Handle => _safeHandle.DangerousGetHandle();

  public void Dispose() => _safeHandle.Dispose();

  /// <summary>
  /// Construct a new parser with no language set.
  ///
  /// Call `Parser.set_language` before parsing.
  /// </summary>
  public static Parser New() {
    var nativeResult = NativeMethods.ParserNew(

    );
    if (nativeResult == IntPtr.Zero) {
      var ec = NativeMethods.LastErrorCode();
      var ctxPtr = NativeMethods.LastErrorContext();
      var msg = global::System.Runtime.InteropServices.Marshal.PtrToStringUTF8(
                    ctxPtr) ??
                "ParserNew failed";
      throw new TreeSitterLanguagePackException(ec, msg);
    }
    var returnValue = new Parser(nativeResult);
    return returnValue;
  }

  /// <summary>
  /// Configure the parser to use the language identified by name (e.g.
  /// `"python"`).
  ///
  /// Resolves the language through the global registry — auto-downloading
  /// if necessary, when the `download` feature is enabled.
  /// </summary>
  public void SetLanguage(string name) {
    NativeMethods.ParserSetLanguage(Handle, name);
    if (NativeMethods.LastErrorCode() != 0) {
      var ec = NativeMethods.LastErrorCode();
      var ctxPtr = NativeMethods.LastErrorContext();
      var msg = global::System.Runtime.InteropServices.Marshal.PtrToStringUTF8(
                    ctxPtr) ??
                "ParserSetLanguage failed";
      throw new TreeSitterLanguagePackException(ec, msg);
    }
  }

  /// <summary>
  /// Parse a UTF-8 source string. Returns `None` if parsing was cancelled
  /// or no language is set.
  /// </summary>
  public Tree? Parse(string source) {
    var nativeResult = NativeMethods.ParserParse(Handle, source);
    if (nativeResult == IntPtr.Zero) {
      return null;
    }
    var returnValue = new Tree(nativeResult);
    return returnValue;
  }

  /// <summary>
  /// Parse a raw byte slice. Returns `None` if parsing was cancelled or
  /// no language is set.
  /// </summary>
  public Tree? ParseBytes(byte[] source) {
    var sourceHandle = GCHandle.Alloc(source, GCHandleType.Pinned);
    var nativeResult = NativeMethods.ParserParseBytes(
        Handle, sourceHandle.AddrOfPinnedObject(), (nuint)source.Length);
    if (nativeResult == IntPtr.Zero) {
      return null;
    }
    var returnValue = new Tree(nativeResult);
    if (sourceHandle.IsAllocated)
      sourceHandle.Free();
    return returnValue;
  }

  /// <summary>
  /// Reset internal state. The next call to `parse` will
  /// not be incremental.
  /// </summary>
  public void Reset() { NativeMethods.ParserReset(Handle); }

  public static Parser Default() {
    var nativeResult = NativeMethods.ParserDefault(

    );
    if (nativeResult == IntPtr.Zero) {
      var ec = NativeMethods.LastErrorCode();
      var ctxPtr = NativeMethods.LastErrorContext();
      var msg = global::System.Runtime.InteropServices.Marshal.PtrToStringUTF8(
                    ctxPtr) ??
                "ParserDefault failed";
      throw new TreeSitterLanguagePackException(ec, msg);
    }
    var returnValue = new Parser(nativeResult);
    return returnValue;
  }
}
