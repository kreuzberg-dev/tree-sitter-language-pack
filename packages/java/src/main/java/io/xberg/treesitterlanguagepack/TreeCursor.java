...
// alef:hash:c740495014f2e86598fa5f909378054afa1687c1cc02d2bddd40e30b308cade3
...
public class TreeCursor implements AutoCloseable {
    ...
    public boolean gotoFirstChild() throws TreeSitterLanguagePackRsException {
        try {
            ...
            try {
            try (HandleLease handleLease = borrowHandle()) {
            var result = (long) NativeLib.TS_PACK_TREE_CURSOR_GOTO_FIRST_CHILD.invoke(handleLease.handle());
            return result != 0;
            }
            } catch (Throwable failure) {
                operationFailure = failure;
    ...
    public boolean gotoParent() throws TreeSitterLanguagePackRsException {
        try {
            ...
            try {
            try (HandleLease handleLease = borrowHandle()) {
            var result = (long) NativeLib.TS_PACK_TREE_CURSOR_GOTO_PARENT.invoke(handleLease.handle());
            return result != 0;
            }
            } catch (Throwable failure) {
                operationFailure = failure;
    ...
    public boolean gotoNextSibling() throws TreeSitterLanguagePackRsException {
        try {
            ...
            try {
            try (HandleLease handleLease = borrowHandle()) {
            var result = (long) NativeLib.TS_PACK_TREE_CURSOR_GOTO_NEXT_SIBLING.invoke(handleLease.handle());
            return result != 0;
            }
            } catch (Throwable failure) {
                operationFailure = failure;
