...
// alef:hash:9dc69b8aa0879ca0d185c59642b8196f385fe458a4cf36b161de68618aadaafa
...
// CaptureOutput controls what data is captured for each query match.
type CaptureOutput string

const (
	// CaptureOutputText capture only the matched text.
	CaptureOutputText CaptureOutput = "text"
	// CaptureOutputNode capture only the `NodeInfo`.
	CaptureOutputNode CaptureOutput = "node"
	// CaptureOutputFull capture both text and `NodeInfo` (default).
...
// ExtractionPattern defines a single extraction pattern and its configuration.
...
// ExtractionPatternOption is an option function for ExtractionPattern.
...
// ExtractionConfig configuration for an extraction run against a single language.
type ExtractionConfig struct {
	// The language name (e.g., `"python"`).
	Language string `json:"language"`
	// Named patterns to run. Keys become the keys in `ExtractionResult::results`.
	Patterns map[string]ExtractionPattern `json:"patterns,omitempty"`
}
...
func WithExtractionConfigPatterns(v map[string]ExtractionPattern) ExtractionConfigOption {
...
// CaptureResult single captured node within a match.
type CaptureResult struct {
	...
	ChildFields map[string]*string `json:"child_fields,omitempty"`
	// Byte offset where this capture starts in the source.
	StartByte uint `json:"start_byte"`
}

// CaptureResultOption is an option function for CaptureResult.
...
func WithCaptureResultChildFields(v map[string]*string) CaptureResultOption {
...
// MatchResult single query match containing one or more captures.
...
// MatchResultOption is an option function for MatchResult.
...
// PatternResult results for a single named pattern.
...
// PatternResultOption is an option function for PatternResult.
...
// ExtractionResult complete extraction results for all patterns.
type ExtractionResult struct {
	// The language that was used.
	Language string `json:"language"`
	// Results keyed by pattern name.
	Results map[string]PatternResult `json:"results,omitempty"`
}
...
func WithExtractionResultResults(v map[string]PatternResult) ExtractionResultOption {
...
// PatternValidation validation information for a single pattern.
...
// PatternValidationOption is an option function for PatternValidation.
...
// ValidationResult validation results for an entire extraction config.
type ValidationResult struct {
	// Whether all patterns are valid.
	Valid bool `json:"valid"`
	// Per-pattern validation details.
	Patterns map[string]PatternValidation `json:"patterns,omitempty"`
}

// ValidationResultOption is an option function for ValidationResult.
...
func WithValidationResultPatterns(v map[string]PatternValidation) ValidationResultOption {
...
type ProcessResult struct {
	...
	Extractions map[string]PatternResult `json:"extractions,omitempty"`
}
...
func WithProcessResultExtractions(v map[string]PatternResult) ProcessResultOption {
...
// NodeInfo lightweight snapshot of a tree-sitter node's properties.
//
...
// NodeInfoOption is an option function for NodeInfo.
...
type ProcessConfig struct {
	...
	Extractions *map[string]ExtractionPattern `json:"extractions,omitempty"`
}
...
func WithProcessConfigExtractions(v map[string]ExtractionPattern) ProcessConfigOption {
...
// QueryMatch single match from a tree-sitter query, with captured nodes.
...
// QueryMatchOption is an option function for QueryMatch.
...
// RootNodeInfo get a `NodeInfo` snapshot of the root node.
...
// FindNodesByType find all nodes matching the given type name, returning their `NodeInfo`.
//
...
// NamedChildrenInfo get `NodeInfo` for all named children of the root node.
//
...
// TreeContainsNodeType check whether any node in the tree matches the given type name.
//
...
// TreeHasErrorNodes check whether the tree contains any ERROR or MISSING nodes.
//
...
// TreeToSexp return the S-expression representation of the entire tree.
//
...
// TreeErrorCount count the number of ERROR and MISSING nodes in the tree.
//
...
// RunQuery execute a tree-sitter query pattern against a parsed tree.
//
...
func RunQuery(tree *Tree, language string, querySource string, source []byte) (*[]QueryMatch, error) {
	...
	cQuerySource := C.CString(querySource)
...
// ExtractPatterns run extraction patterns against source code.
//
...
// ValidateExtraction validate extraction patterns without running them.
//
