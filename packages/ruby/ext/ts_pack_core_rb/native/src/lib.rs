...
fn process(source: String, config: String) -> Result<ProcessResult, Error> {
    let config_core: tree_sitter_language_pack::ProcessConfig = serde_json::from_str(&config).map_err(|e| {
        magnus::Error::new(
            unsafe { Ruby::get_unchecked() }.exception_runtime_error(),
            e.to_string(),
        )
...
fn extract_patterns(source: String, config: String) -> Result<ExtractionResult, Error> {
    let config_core: tree_sitter_language_pack::ExtractionConfig = serde_json::from_str(&config).map_err(|e| {
        magnus::Error::new(
            unsafe { Ruby::get_unchecked() }.exception_runtime_error(),
            e.to_string(),
        )
...
fn configure(config: String) -> Result<(), Error> {
    let config_core: tree_sitter_language_pack::PackConfig = serde_json::from_str(&config).map_err(|e| {
        magnus::Error::new(
            unsafe { Ruby::get_unchecked() }.exception_runtime_error(),
            e.to_string(),
        )
