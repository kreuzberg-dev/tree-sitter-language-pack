use crate::fixtures::{Fixture, escape_c_string, group_by_category, sanitize_name};
...
fn write_cmake_lists(dir: &Path) -> Result<(), String> {
...
fn write_test_file(dir: &Path, category: &str, fixtures: &[&Fixture]) -> Result<(), String> {
    ...
    for fixture in fixtures {
        ...
        writeln!(out, "static void {fn_name}(void) {{").unwrap();
            ...
            && let Some(req_lang) = &skip.requires_language
        {
            ...
            writeln!(
                out,
                "        printf(\"SKIP: %s (language '{}' not available)\\n\", __func__);",
                escape_c_string(req_lang)
            )
            .unwrap();
