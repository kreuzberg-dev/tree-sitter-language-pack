...
| [`mcp`](/guides/mcp-server/)          | Run the MCP server for AI agents          |
...
| `--groups <groups>` | Download by group (comma-separated). Only manifest-defined names are accepted; the published manifest defines exactly one, `all`. |
...
Known:       true
Downloaded:  true
Cache path:  /home/user/.cache/tree-sitter-language-pack/v1.14.3/libs/libtree_sitter_python.so
```
...
```bash
ts-pack cache-dir
# /home/user/.cache/tree-sitter-language-pack/v1.14.3/libs
