...
init(PackConfig(groups=["web"]))      # JavaScript, TypeScript, HTML, CSS
init(PackConfig(groups=["systems"]))  # C, C++, Rust, Go
init(PackConfig(groups=["data"]))     # Python, R, SQL, JSON

# Combine languages and groups
init(PackConfig(languages=["python"], groups=["web", "systems"]))
...
`PackConfig::from_toml_file()` and `PackConfig::discover()` exist only on the Rust
`PackConfig` (behind the `config` feature). They are not exported to Python or any other
binding — the CLI (`ts-pack download`) reads `language-pack.toml` on your behalf.
:::
