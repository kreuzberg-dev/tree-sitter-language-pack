...
<!-- alef:hash:0f0f9e876b424373ba8f4cb344bdde77614109ddbfdcb103f8a06a16af30e897 -->
...
| `groups` | `--groups` | `Vec<String>` |  | Download language groups (comma-separated). The manifest currently defines exactly one group, `all`; enumerate the real names with `manifest_groups()`. |
...
| Name | Flags | Type | Default | Description |
| --- | --- | --- | --- | --- |
| `cache_dir` | `--cache-dir` | `Option<String>` |  | Base directory for the parser cache (a versioned subdirectory is created under it) |
