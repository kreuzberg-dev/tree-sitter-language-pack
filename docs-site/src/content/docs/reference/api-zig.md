...
<!-- alef:hash:ffed43e95095f43188959b5cef08efbd2535c822cdd04e4f2d69f0c92db19e68 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Zig API Reference <span class="version-badge">v1.15.11</span>
...
```zig
pub fn manifest_groups() Error![]const [:0]const u8
```
...
| `node_types` | `\[\]const \[:0\]const u8` | `[]` | Tree-sitter node kinds that appear at the top level of this chunk. |
| `context_path` | `\[\]const \[:0\]const u8` | `[]` | Hierarchical path of enclosing structural items (e.g., `["MyClass", "my_method"]`). |
| `symbols_defined` | `\[\]const \[:0\]const u8` | `[]` | Names of symbols defined within this chunk. |
...
| `associated_node` | `\[:0\]const u8?` | `null` | Name of the syntax node this comment is directly associated with. |
...
| `associated_item` | `\[:0\]const u8?` | `null` | Name of the item this docstring documents. |
...
```zig
pub fn new_download_manager(version: [:0]const u8) Error!DownloadManager
```
...
```zig
pub fn installed_languages(self: *const DownloadManager) []const [:0]const u8
```
...
| `items` | `\[\]const \[:0\]const u8` | `[]` | Specific names imported from the source module. For `import a, b` / `from m import a, b`, every entry's base name (never the alias). For a JavaScript/TypeScript named-imports clause (`import { a, b as c } from 'm'`), every specifier's original name. Populated for Python and JavaScript/TypeScript only. Always empty for every other language this library recognises import statements for — Rust, Go, Java, Kotlin, and Elixir (`import`/`alias`/`require`/`use`) — and for a JS/TS namespace import (`import * as ns`) or default import (`import x from 'm'`), neither of which names individual items. |
| `alias` | `\[:0\]const u8?` | `null` | Alias assigned to the import (e.g., `import numpy as np`). Populated for Python's single-name form (`import numpy as np`, `from m import a as b`) and JavaScript/TypeScript's namespace form (`import * as ns from 'm'`) or a named-imports clause naming exactly one specifier (`import { a as b } from 'm'`). `null` for Rust, Go, Java, Kotlin, and Elixir (which has its own `as:` alias option on `alias` directives, not yet extracted here) — and for a Python statement that aliases several names at once (`import os, sys as s`), where there is no single alias to report for the statement as a whole, so only `items` is populated for it. |
...
```zig
pub fn get_language(self: *const LanguageRegistry, name: [:0]const u8) Error!Language
```
...
```zig
pub fn available_languages(self: *const LanguageRegistry) []const [:0]const u8
```
...
```zig
pub fn has_parser(self: *const LanguageRegistry, name: [:0]const u8) bool
```
...
```zig
pub fn has_language(self: *const LanguageRegistry, name: [:0]const u8) bool
```
...
```zig
pub fn byte_range(self: *const Node) ByteRange
```
...
```zig
pub fn start_position(self: *const Node) Point
```
...
```zig
pub fn end_position(self: *const Node) Point
```
...
```zig
pub fn child_by_field_name(self: *const Node, name: [:0]const u8) ?Node
```
...
```zig
pub fn to_sexp(self: *const Node) [:0]const u8
```
...
```zig
pub fn set_language(self: *const Parser, name: [:0]const u8) Error!void
```
...
```zig
pub fn field_name(self: *const TreeCursor) ?[:0]const u8
```
