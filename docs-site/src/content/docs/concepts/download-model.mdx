...
```python
from tree_sitter_language_pack import PackConfig, download, download_all, init
...
init(PackConfig(languages=["python", "javascript"]))
```
...
// Configure + download in one call
init({ languages: ["python", "javascript"] });
```
...
```rust
use tree_sitter_language_pack::{PackConfig, download, download_all, init};
...
// Configure + download in one call
init(&PackConfig {
    languages: Some(vec!["python".to_string(), "javascript".to_string()]),
    ..Default::default()
})?;
```
