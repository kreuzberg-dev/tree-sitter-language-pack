...
// alef:hash:61c62059d952c16c9983f1c61dad8635a80862463be955608d85951b16740366
...
fn main() {
    ...
    // `#if defined(TS_PACK_FEATURE_X)` (its `[defines]` table), and it does so from
    ...
    // carrying `.` or `+`, which Cargo leaves untouched in the variable name.
    {
        ...
        if !header.contains(marker) && header.contains("TS_PACK_FEATURE_") {
            ...
            let mut defines: Vec<String> = std::env::vars_os()
                .filter_map(|(key, _)| {
                    ...
                    Some(format!("#define TS_PACK_FEATURE_{name} 1"))
                })
                .collect();
            defines.sort();
            defines.dedup();
    ...
    // `binding.go` carries `#include "ts_pack.h"` under cgo, so the header has to be
