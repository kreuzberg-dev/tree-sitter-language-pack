...
// alef:hash:c5d2754b6110412d592ed37f4d604e6792fc3238c6ad0f33818cc74b5a61aa56
...
fn export_generated_headers() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut header = String::from_utf8(bytes).map_err(|error| {
        io::Error::new(
            ...
            format!("cbindgen generated invalid UTF-8 for include/ts_pack.h: {error}"),
        )
    })?;
    header = header.replace("TS_PACKTSLanguage", "TSLanguage");
    stamp_enabled_features(&mut header);
    let destinations = [
        Path::new("include/ts_pack.h"),
        Path::new("../../packages/go/include/ts_pack.h"),
    ];
